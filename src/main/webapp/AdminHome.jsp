  ...
  <section class="container p-4">
    <%
    	...
    	%>
		    	...
		    	<a class="btn btn-sm btn-secondary" href="ViewIDProof?email=<%= carOwner.get("email")%>" target="_blank">View ID Proof</a>
		    	
		    <%	
		    		String status=(String)carOwner.get("status");
		    		if(status.equalsIgnoreCase("Pending")){
		    %>
		    			<a class="btn btn-sm btn-success" href="ChangeCarOwnerStatus?email=<%= carOwner.get("email")%>&status=Accept">Accept</a>
		    			<a class="btn btn-sm btn-danger" href="ChangeCarOwnerStatus?email=<%= carOwner.get("email")%>&status=Reject">Reject</a>
		    <%			
		    		}
		    	%>
		    	</p>	
